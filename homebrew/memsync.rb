class Memsync < Formula
  ...
  def install
    bin.install "bin/memsync"
    (share/"memsync").install "templates"
    ...
    inreplace bin/"memsync",
              'TEMPLATE_DIR="$(dirname "$SCRIPT_DIR")/templates"',
              "TEMPLATE_DIR=\"#{share}/memsync/templates\""
  end

  test do
    assert_match "memsync #{version}", shell_output("#{bin}/memsync --version")
    assert_match "USAGE", shell_output("#{bin}/memsync --help")

    # init must create the full scaffold in a fresh directory
    mkdir testpath/"target"
    system bin/"memsync", "init", "--target", testpath/"target"
    %w[
      .ai/README.md
      .ai/project.md
      .ai/architecture.md
    ...
    system bin/"memsync", "check", testpath/"target"
  end
end
