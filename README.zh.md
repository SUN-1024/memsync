...
memsync init      # 写入 .ai/、CLAUDE.md、AGENTS.md(已存在的文件会被跳过)
memsync check     # 校验脚手架是否完整、非空
...
脚本会下载最新发布的 tarball,把 `memsync` 装到 `/usr/local/bin/memsync`,
templates 装到 `/usr/local/share/memsync/templates`。仅当目标前缀不可写时
...
npm 包名是 `memsync-cli`(npm 上的 `memsync` 已被占用),装好之后命令仍是
`memsync`:
