...
test_init_creates_all_files() {
  local out rc
  out="$(bash "$REPOMEMO" init --target "$WORK" 2>&1)"
  rc=$?
...
test_init_idempotent() {
  bash "$REPOMEMO" init --target "$WORK" >/dev/null 2>&1 \
...
test_init_force_overwrites() {
  bash "$REPOMEMO" init --target "$WORK" >/dev/null 2>&1 || return 1

  # Mutate one of the files so we can detect the overwrite.
  echo "MUTATED" > "$WORK/.ai/handoff.md"

  local out rc
  out="$(bash "$REPOMEMO" init --target "$WORK" --force 2>&1)"
  rc=$?
  assert_eq "$rc" "0" "exit code" || return 1
...
test_check_passes_after_init() {
  bash "$REPOMEMO" init --target "$WORK" >/dev/null 2>&1 || return 1
  local out rc
  out="$(bash "$REPOMEMO" check "$WORK" 2>&1)"
  rc=$?
  assert_eq "$rc" "0" "exit code" || { echo "$out" | sed 's/^/    /' >&2; return 1; }
...
test_self_check() {
  local out rc
  out="$(bash "$REPOMEMO" check "$ROOT" 2>&1)"
  rc=$?
  assert_eq "$rc" "0" "exit code on self-check" \
    || { echo "$out" | sed 's/^/    /' >&2; return 1; }
}

echo "running repomemo integration tests against $REPOMEMO"
echo
